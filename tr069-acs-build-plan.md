...
## 10. Current status & outstanding items — corrected 2026-08-11, updated same day (§11)

Supersedes §8 and every scattered "flagged as an open item" callout in §§1-9 above; this is the accurate current list, cross-checked against the actual code (not just prior documentation) as of 2026-08-11. `go build ./...`, `go vet ./...` (backend) and `tsc -b` (frontend) all pass clean. **§11 below is a same-day follow-up pass that closes several of the items this section originally listed as open — read §11 for what changed; the items below are left as originally written except where struck through inline, so the history stays honest.**
...
- **`data_model_root` branching across hardcoded `Device.*` write paths** (TR-181 vs legacy TR-098 `InternetGatewayDevice.`) — **partially closed, see §11.** A real canonical-parameter resolver now exists and is wired into diagnostics (ping/traceroute), Connection Request credential rotation, the WiFi-clients refresh, and the BSS `MODIFY_WIFI` template — the concrete paths this codebase already hardcoded. Deliberately **not** closed: an operator-typed arbitrary path (console/REST `SET_PARAMETER`, `AddObject`/`DeleteObject`) is still sent exactly as typed, never rewritten — the operator already knows their device's real tree from parameter discovery, and silently translating a path they explicitly chose would be surprising. That's a real, narrower remaining gap, not the whole systemic one this bullet originally described.
...
- **Per-vendor canonical-parameter registry** (gap analysis item 13, §"Nice-to-have feature backlog") — still not built; needs real vendor documentation this project doesn't have. Parameter discovery is a related, shipped mitigation, not a substitute. (Not the same thing as §11's canonical *path* resolver, which is TR-181/TR-098 root-branching, not per-vendor parameter naming.)
- ~~`SetParameterAttributes`/`GetParameterAttributes` — REST endpoints exist, no frontend UI.~~ **Closed, see §11.**
- ~~OpenAPI spec — still deferred.~~ **Closed, see §11.**
- ~~`ParameterKey`/Inform event-code correlation — never consumed on a `4 VALUE CHANGE` Inform event.~~ **Closed, see §11.**
...
- **No CI/CD, no container images, thin test coverage** — flagged by an external gap review the same day this section was written, not by this document originally. **Closed, see §11.**
...
## 11. Gap-remediation pass — 2026-08-11 (same day as §10)

A follow-up pass closing the buildable items §10 flagged, requested directly against an external gap-analysis review of the whole codebase (not derived from v3 or this doc). Scope was explicitly agreed with the user first (quick fixes/CI, UI/doc completions, test coverage, and the `data_model_root` refactor) rather than assumed — the last of those is the kind of "systemic refactor... needing full regression verification" this document has repeatedly flagged as too risky to do speculatively, so it got real thought rather than a blind sweep.

**`ParameterKey`/`4 VALUE CHANGE` correlation** (`cmd/acs/enforce.go`'s new `correlateValueChangeEvents`, called from `handleInform`): when a CPE echoes a `SetParameterValues`' `ParameterKey` back as an Inform `EventStruct.CommandKey` on a `4 VALUE CHANGE` event, the job is now looked up and an audit record (`ValueChangeConfirmed`) written — out-of-band confirmation that the CPE itself flagged the change, distinct from the ordinary `GetParameterValues` auto-confirm. An empty `CommandKey` (a change the ACS didn't cause) gets its own `UnsolicitedValueChange` audit entry instead — useful drift/tampering signal now that `SetParameterAttributes` (below) makes active notification actually reachable from the console.

**`data_model_root` canonical-path resolver** (new `internal/devices/adapters/canonical.go`) — implements design doc v3 §6.2/§6.3's `CanonicalParameter`/`resolve_path` exactly as originally specified, the piece `catalog.go`'s own doc comment had flagged as "later-phase work" since Phase 2. `ResolvePath(root, canonical)` maps a fixed, small set of canonical parameters (software version, the four `ManagementServer.*` Connection Request fields, WiFi SSID/passphrase) to their TR-181 or TR-098 path; `DiagnosticsPrefix(root, kind)` and `WiFiAssociatedDevicesPrefix(root)` handle the ping/traceroute and WiFi-sweep cases, which move between different subtrees rather than just swapping a root prefix. Wired into every concrete write/read path this codebase already hardcoded: `createDiagnosticsPing`/`createDiagnosticsTraceroute` resolve and store the prefix on the job payload *at creation time* (same precedent as `FirmwareDownloadPayload` — resolved once, not re-resolved at dispatch, so a poll minutes later still targets the subtree the trigger did even if discovery changes the device's root in between); `refreshWifiClients`, `rotateDeviceCredential`, and the scheduled-diagnostics fan-out (`schedule_worker.go`, resolved *per device* since a group target can span mixed roots) all do the same. `internal/bss/template.go`'s `Translate` gained a `dataModelRoot` parameter, resolved by a new `ACSClient.GetDevice` call — but only for `MODIFY_WIFI`, not `SUSPEND`/`ACTIVATE`, so the walled-garden path doesn't pay for an extra internal-API round-trip or gain a new failure mode it didn't have. IGD:1's WiFi paths assume `LANDevice.1`/`WLANConfiguration.1` — the near-universal single-primary-radio convention, not a spec guarantee — documented as the honest limit of this pass, the same way the per-vendor registry gap above is.

**Frontend `SetParameterAttributes`/`GetParameterAttributes` UI**: `setParameterAttributes` existed in `api/client.ts` since some earlier pass but was never called from anywhere; `getParameterAttributes` didn't exist at all. Both now have a "Parameter attributes" panel in Device Detail (set a path + notification level 0/1/2; read a comma-separated path list back), and Recent Jobs renders a `GET_PARAMETER_ATTRIBUTES` job's `result_detail` inline (`path=notification` pairs), the same pattern `instance_number` already used for `ADD_OBJECT`.

**OpenAPI specs** (`backend/openapi.yaml` for `cmd/api`, `backend/openapi-bssadapter.yaml` for `cmd/bssadapter`) — hand-written against the actual route table and request/response structs (not generated), covering the full path set for both services with reusable component schemas. Validated structurally clean with `@redocly/cli lint` (zero errors; only cosmetic warnings — missing `operationId`/license field/4xx-on-every-op — left as-is for a hand-written internal spec). Documents the plain-text-vs-JSON-envelope error-response difference between the two services explicitly, since that's a real, easy-to-miss integration detail.

**CI/CD and containerization** — both flagged as complete gaps, not previously anything to "supersede": `.github/workflows/acs-ci.yml` (path-filtered to `ACS/**` since this lives in a monorepo with unrelated sibling projects) runs `go build`/`go vet`/`go test -race` and `tsc -b`/`oxlint`/`npm test` on every push/PR — enforcing, not just narrating, the "all clean" claims this document makes at every milestone. `backend/Dockerfile.{acs,api,bssadapter}` (multi-stage, `CGO_ENABLED=0` static build → `distroless/static-debian12:nonroot`) and `frontend/Dockerfile` (multi-stage → nginx) close `deployment-testing-onboarding-guide.md` §1's "no container image for them yet" — added to `infra/docker-compose.yml` behind an opt-in `containerized` profile so the documented default `go run`/systemd workflow is completely unchanged; `infra/prometheus.yml` scrapes both the host-based and containerized target names so either deployment mode "just works" without editing config. Static cross-compiles verified locally (`GOOS=linux CGO_ENABLED=0 go build` for all three); the actual `docker build`/`docker compose up` couldn't be exercised in this environment (no running Docker daemon) — a real gap in *this pass's* verification, flagged rather than silently assumed.

**Backend test coverage**: seven new test files targeting pure, DB-independent logic that had zero coverage — `internal/rollout` (`InMaintenanceWindow`'s same-day/overnight-wrap/unconfigured/unparseable-fails-open cases, `deriveState`'s full job-status mapping), `internal/firmware` and `internal/uploads` (`Storage.Save`/`Open` round-trips via `t.TempDir()`, `HashReader`'s streamed SHA256), `internal/operators` (`AtLeast`'s full rank matrix — the actual enforcement boundary behind every `route()` registration's `minRole`, previously untested despite being security-critical), `internal/mailer` (`Configured()`, the unconfigured-logs-instead-of-sending fallback, and that a configured `Mailer` actually attempts real delivery rather than silently falling back), and `internal/jobs` (`newCommandKey`'s prefix-per-type mapping and uniqueness). Packages requiring a live Postgres (most REST handlers, most `Repository` methods) were left alone — no DB was available in this environment to integration-test against, and mocking `*sql.DB` throughout wasn't judged worth the risk/diff size for this pass.

**Frontend test framework**: previously zero automated frontend tests existed (Playwright was a dev dependency used only for ad hoc manual verification sessions, never a committed spec suite). Added Vitest + `@testing-library/react` (`vite.config.ts`'s `test` block, `jsdom` environment; `npm test` runs it), plus 32 tests across `lib/format.ts` (time/duration formatting edge cases), `auth/roles.ts` (the client-side `canWrite`/`canAdmin` UX gate), `auth/tokenStore.ts` (the module-level auth singleton — reload/persistence/idempotency behavior, using `vi.resetModules()` to get a fresh instance per test), and a `StatusBadge` component render test proving the React Testing Library path works end to end, not just pure-function tests.

**Real bug caught while writing those tests, not by inspection**: `tokenStore.ts`'s `markAuthRequired()` — the function that learns "this backend requires auth" from a `401`, as opposed to `setAuthenticated`'s "learns it from a successful login" — mutated state and notified listeners but never called `persist()`. The module's own doc comment states the invariant plainly: "once true, stays true... across reloads," and Phase 6's frontend section documents fixing exactly this class of bug for the login-success path. The 401-discovery path had the identical bug, just never exercised by that verification pass (it tested via a real login, not via a stale/pre-auth client hitting a 401 first). A test asserting the documented invariant against the 401 path failed immediately; one-line fix (`persist()` added to `markAuthRequired`), now covered.

**Verified**: `go build ./...`, `go vet ./...`, and `go test ./...` clean across all packages (27 test files, up from 20). Frontend `tsc -b`, `npm run build`, `npx oxlint` (no new warnings beyond five pre-existing ones unrelated to this pass), and `npm test -- --run` (32/32 passing, matching the CI workflow's exact invocation) all clean. Both OpenAPI specs pass `@redocly/cli lint` with zero structural errors. `docker compose config` (both with and without `--profile containerized`) parses all seven services correctly.

**Not attempted this pass, stated plainly**: actually running `docker build`/`docker compose up` against a live daemon (none available here); the full `data_model_root` refactor across every conceivable write path (only the concrete hardcoded ones); DB-backed integration tests (no Postgres available here — see the CI workflow for what a real environment should add: a `postgres` service container plus `internal/store.Migrate` before `go test`, following the same pattern `infra/docker-compose.yml` already uses for local dev).
