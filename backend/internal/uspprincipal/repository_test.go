...
func TestCertificatePrincipalAuthenticateRotateAndDisable(t *testing.T) {
	...
	if _, err := principals.AuthenticateCertificate(ctx, certA); !errors.Is(err, principal.ErrNotFound) {
	...
	if _, err := principals.AuthenticateCertificate(ctx, certB); !errors.Is(err, principal.ErrNotFound) {
