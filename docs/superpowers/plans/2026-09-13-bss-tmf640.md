...
>
> **Why now:** C-3 (TMF638), C-4 (TMF641) and C-5 (TMF642/688) all render
> the same SID `Service` resource. If this task writes the serializer into
> `cmd/bssadapter`, C-3 either duplicates it — and the two drift the first
> time one is patched — or refactors it out later. `tmf640.go` does not
> exist yet, so making the split here is free.
>
> **What changes, concretely:**
> - Create `backend/internal/tmf/service.go` + `service_test.go` instead of
>   putting the types in `tmf640.go`. The package has **no HTTP handler and
...
- Create: `backend/cmd/bssadapter/tmf640.go`
