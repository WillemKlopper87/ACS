...
- Produces: `tmfService`, `tmfServiceCharacteristic`, `tmfRelatedParty` types; `tmfServiceBasePath`/`tmfMonitorBasePath` constants; `handler.serviceFromMapping(ctx, m *bss.AccountDeviceMapping) (tmfService, error)`; `handler.listServices`, `handler.getService` HTTP handlers — `tmfServiceBasePath`/`tmfMonitorBasePath` and the `tmfService`/`tmfServiceCharacteristic` types are consumed by exact name in Tasks 5 and 6.
