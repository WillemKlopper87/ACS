...
- a Digest response's `uri` is accepted whether it echoes the full request target or only the bare path — some CPE HTTP stacks drop a query string they were challenged on when answering the 401;
- device identity (`oui_serial`) normalizes OUI case and separators (`"aa:bb:cc"`/`"AABBCC"`/etc. all match) before matching or storing it, so the same physical device can't split into two rows depending on which form its firmware happens to send on a given contact;
- `SetParameterValues` always tags a value `xsi:type="xsd:string"`, never a parameter's real declared type — some CPE firmware (Huawei's `cwmpmng`, per FreeACS's own documented fix for the analogous `NextLevel` RPC parameter) strictly enforces `xsd:boolean`'s `"0"`/`"1"` numeric lexical form and rejects the word form outright, so a boolean-typed parameter (e.g. a walled-garden toggle) must be written as `"0"`/`"1"`, not `"true"`/`"false"` — `bssadapter` warns at startup if `ACS_WALLED_GARDEN_SUSPEND_VALUE`/`ACS_WALLED_GARDEN_ACTIVE_VALUE` look like a word-form boolean;
...
| `ACS_WALLED_GARDEN_SUSPEND_VALUE` / `ACS_WALLED_GARDEN_ACTIVE_VALUE` | unset — SUSPEND/ACTIVATE rejected until configured (`bssadapter`, build plan §5.3) | Use `"0"`/`"1"` for a boolean-typed walled-garden parameter, never `"true"`/`"false"`/`"yes"`/`"no"` — some CPE firmware strictly enforces the numeric lexical form and rejects the word form outright. `bssadapter` logs a startup warning if either value looks word-form. |
